...
V(net)$indeg <- igraph::degree(net, mode="in", normalized = T)+1
...
maincore <- induced_subgraph(main, V(main)$core %in% c(2:1057))
...
sum(table(V(state.main)$core)[-c(1:13)])/sum(table(V(state.main)$core)) # 20%
sum(table(V(state.main)$core)[-c(1:13)]) # 99
table(V(state.main)$core)[c(1:13)]
state.main.core <- induced_subgraph(state.main, V(state.main)$core %in% c(15:1048))
...
V(region.net)$indeg <- (igraph::degree(region.net, mode="in", normalized = T))*10
...
sum(table(V(region.main)$core)[-c(1)])/sum(table(V(region.main)$core))
sum(table(V(region.main)$core)[-c(1)])
table(V(region.main)$core)[c(1)]
region.main.core <- induced_subgraph(region.main, V(region.main)$core %in% c(3:151))
...
V(county.net)$indeg <- (igraph::degree(county.net, mode="in", normalized = T))*10
...
sum(table(V(county.main)$core)[-c(1:3)])
table(V(county.main)$core)[c(1:3)]
county.main.core <- induced_subgraph(county.main, V(county.main)$core %in% c(5:71))
...
ggsave(filename = "plots/alt-fig6_networks-traditional.png", width = 10, height = 3)
