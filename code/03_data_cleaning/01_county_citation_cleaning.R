...
county_nolayout <- readRDS("~/Box/truckee/data/compiled_anystyle_results/county_documents_nolayout_references.RDS")
county <- readRDS("~/Box/truckee/data/compiled_anystyle_results/county_documents_references.RDS")
...
same <- intersect(names(county_nolayout), names(county))
combined <- full_join(county_nolayout, county, by = same) 
...
# Need to assign new rows because they have previously been messed up with the unnesting -- there are now duplicates
df$row <- 1:nrow(df)
...
# Once again need to re-row, because the widening made the row order change, and now it will not match with seq_along.Though I could just take row number in the loop, but nah
df$row <- 1:nrow(df)
...
write.csv(refined.df, "~/Box/truckee/data/compiled_anystyle_results/county_refined.csv", row.names = F)
