...
mpo_nolayout <- readRDS("~/Box/truckee/data/compiled_anystyle_results/mpo_documents_nolayout_references.RDS")
mpo <- readRDS("~/Box/truckee/data/compiled_anystyle_results/mpo_documents_references.RDS")
...
same <- intersect(names(mpo_nolayout), names(mpo))
combined <- full_join(mpo_nolayout, mpo, by = same) 
...
write.csv(refined.df, "~/Box/truckee/data/compiled_anystyle_results/mpo_refined.csv", row.names = F)
