...
sapply(gsub('^documents','reference_extracts',big_dirs[!grepl('ceqa',big_dirs)]),dir.create,recursive = T)
...
pblapply(seq_along(json_files[still_need]),function(i){
  system(paste('anystyle --overwrite -f json,xml,csl find',fls[still_need][i],' ',json_dirs[still_need][i]))
},cl = 4)
}
